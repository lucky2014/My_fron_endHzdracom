;(function($){ 
	...
	function getData(settings,dom,flag,countryDom){
		$.ajax({
			...
			success:function(result){
				if (result.resultCode==1000 && result.returnObject.length>0) {
					var str = "";
					for(var i=0;i<result.returnObject.length;i++){
						str += '<li data-name="'+result.returnObject[i].areaName+'" parentId="'+result.returnObject[i].id+'">'+result.returnObject[i].areaName+'</li>';
					}
	...
	function liClick(settings,dom,flag,cityDom,countryDom){
		dom.delegate("li","click",function(e){
	        ...
	        var parentId = me.attr("parentId");

	        dom.find("i").text(me.text()).attr("parentId",parentId);
	        me.parent().addClass("hide");

	        settings.data = {parentId: parentId};
